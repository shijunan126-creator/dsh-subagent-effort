 ...
 * and resolves externals through the injected require (loader module table 鈥? * cordis DI entities, no globals, no import map). CSS Modules are compiled by
 ...
 * identity to share 鈥?the framework itself is a platform module (external),
 ...
 * five importers (locale, ui-layout, ui-conversation 脳3) ride this single
 ...
 * workspace layout, so the lib half must be restated here 鈥?dropping it leaves
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    // off 鈥?a default clean would wipe the node-half output emitted above.
    outDir: 'lib',
    format: 'cjs',
    platform: 'browser',
    ...
    // import.meta.env.MODE, which a CJS output cannot carry 鈥?rolldown flags
    ...
    // loader module table must inline instead (wire/type layers, zod, clsx 鈥?    // every non-shared dep). A require() the table cannot answer is a
    ...
    plugins: [{
      ...
      // and every other @deepseek-ai value import is a build error 鈥?a
      ...
      resolveId(source: string) {
        ...
        throw new Error(
          `client bundle purity: "${source}" is not a platform module (CLIENT_EXTERNALS), an inline-safe wire layer, or a generated /remote contribution 鈥?`
